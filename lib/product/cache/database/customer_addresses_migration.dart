...
class CustomerAddressesMigration extends Migration {
  ...
  Future<void> up(Transaction txn) async {
    // This method creates the 'customer_addresses' table in the local Sqflite database.
    // Column names are aligned with the aliases used in your SQL SELECT query for consistency.
    await txn.execute(
      DatabaseHelper.createTableSql(
         autoIncrement: false,
        'customer_addresses', // The table name in Sqflite.
        {
    ...
    AppLogger.info('CustomerAddressesMigration: customer_addresses table successfully created with aliased columns.');
  }

  @override
  Future<void> down(Transaction txn) async {
    // This method handles the dropping (deletion) of the 'customer_addresses' table.
    await txn.execute('DROP TABLE IF EXISTS customer_addresses');
    AppLogger.info('CustomerAddressesMigration: customer_addresses table dropped.');
  }
}
