...
class StockMovementsMigration extends Migration {
  ...
  Future<void> up(Transaction txn) async {
    // This method creates the 'stock_movements' table in the local Sqflite database.
    // Column names are aligned with the aliases used in your SQL SELECT query for consistency.
    await txn.execute(
      DatabaseHelper.createTableSql(
        autoIncrement: false,
        'stock_movements', // The table name in Sqflite.
        {
          // Column definitions using the preferred aliased names
          'id': String, // Corresponds to sth_Guid AS id (GUIDs stored as String)
          'type': int, // Corresponds to sth_tip AS type
          'kind': int, // Corresponds to sth_cins AS kind
          'documentType': int, // Corresponds to sth_evraktip AS documentType
          'documentSerialNo': String, // Corresponds to sth_evrakno_seri AS documentSerialNo
          'documentSequenceNo': int, // Corresponds to sth_evrakno_sira AS documentSequenceNo
          ...
          'unitPointer': int, // Corresponds to sth_birim_pntr AS unitPointer
          ...
          'taxPointer': int, // Corresponds to sth_vergi_pntr AS taxPointer
          ...
          'entryWarehouseNo': int, // Corresponds to sth_giris_depo_no AS entryWarehouseNo
          'exitWarehouseNo': int, // Corresponds to sth_cikis_depo_no AS exitWarehouseNo
          ...
          'createdBy': int, // Corresponds to sth_create_user AS createdBy
          'createdAt': String, // Corresponds to sth_create_date AS createdAt
          'updatedBy': int, // Corresponds to sth_lastup_user AS updatedBy
    ...
    AppLogger.info('StockMovementsMigration: stock_movements table successfully created with aliased columns.');
  }

  @override
  Future<void> down(Transaction txn) async {
    // This method handles the dropping (deletion) of the 'stock_movements' table.
    await txn.execute('DROP TABLE IF EXISTS stock_movements');
    AppLogger.info('StockMovementsMigration: stock_movements table dropped.');
  }
}
