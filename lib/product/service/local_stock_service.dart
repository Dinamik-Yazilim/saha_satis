...
import '../../../product/cache/models/app_setting_model.dart';

enum StockSearchType {
  stockCode,
  barcode,
  stockName;
...
class LocalStockService {
  ...
  Future<List<AppSettingModel>> _getAppSetting(Database db) async {
    final List<Map<String, dynamic>> maps = await db.query('app_settings');

    if (maps.isEmpty) {
      return [AppSettingModel(isRetail: 0, terminalDepotNo: 1)];
    }
    return List.generate(maps.length, (i) {
      return AppSettingModel.fromMap(maps[i]);
    });
  }

  Future<List<StockCardModel>> getFilteredStock({
    ...
    if (searchType != null && searchText != null) {
      if (searchType == StockSearchType.stockCode) {
        queryAdditional = '''
          (S.code LIKE ? OR S.code LIKE ? OR S.code LIKE ?)
        ''';
        queryParams.add("%${searchText.toLowerCase()}%");
        queryParams.add("%${searchText.toUpperCase()}%");
        queryParams.add("%$searchText%");
      } else if (searchType == StockSearchType.barcode) {
        queryAdditional = '''
          (CASE
            WHEN BT.content = 1 THEN SUBSTR(BT.code, 1, 7) = SUBSTR(?, 1, 7) OR
              (BT.code LIKE ? AND NOT EXISTS
                (SELECT 1 FROM barcodes WHERE code = ?)) -- 'barcode' kolon adını 'code' olarak düzelttim
            ELSE BT.code = ? OR
              (BT.code LIKE ? AND NOT EXISTS
                (SELECT 1 FROM barcodes WHERE code = ?)) -- 'barcode' kolon adını 'code' olarak düzelttim
           END)
        ...
        queryParams.add("$searchText%");
        queryParams.add(searchText);
      } else if (searchType == StockSearchType.stockName) {
        queryAdditional = '''
    ...
    AppSettingModel appSettingModel = (await _getAppSetting(db)).first;
    int selectedTaxType = appSettingModel.isRetail ?? 0;
    int terminalDepotNo = appSettingModel.terminalDepotNo ?? 1;
...
''';
    ...
    if (queryParams.isNotEmpty) {
      ...
      result = await db.rawQuery(sql); // Parametre göndermemize gerek yok
    }

    List<StockCardModel> stockList;
    stockList =
        result.map((map) {
          ...
          if (selectedTaxType == 1) {
            ...
            mutableMap['PriceIncludingTax'] = mutableMap['WholesalePriceIncludingTax'];
            mutableMap['PriceExcludingTax'] = mutableMap['WholesalePriceExcludingTax'];
          }

          return StockCardModel.fromJson(mutableMap.cast<String, dynamic>());
        }).toList();
